...
import model, math, time
import ai_state_space_generator, view
...
MAXIMUM_STATES_FOR_FORWARD_PRUNING = 5
...
def get_next_move_and_state_from_ai_search(player, state_from, move_number):
    ...
    iterative_deepening_search_with_time_constraint(player, state_from)
...
def get_predefined_playbook_move(player):
    pass

# Iterative deepening tree search with a given time constraint.
def iterative_deepening_search_with_time_constraint(player, state_from):
    ...
    for depth in range(MINIMUM_DEPTH_FOR_ITERATIVE_DEEPENING_SEARCH, MAXIMUM_DEPTH_FOR_ITERATIVE_DEEPENING_SEARCH):
        ...
        # Time-over setting.
        if model.global_game_configuration[player]['time_limitation'] \
...
def max_value(player, state_from, alpha, beta, depth):
    ...
    for successor_state in generate_pruned_and_ordered_next_moves_and_states(player, state_from)[1]:
        value = max(value, min_value(player, successor_state, alpha, beta, depth))
        if value >= beta:
            return value
